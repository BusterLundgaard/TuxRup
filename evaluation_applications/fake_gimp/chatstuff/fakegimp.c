...
static void activate ( GApplication *app, G_GNUC_UNUSED gpointer *data )
{
	...
	GMenu* toolbar = create_toolbar(app);
	enable_toolbar(GTK_APPLICATION(app), GTK_WINDOW(win), toolbar);
	...
	gtk_paned_set_start_child(GTK_PANED(left_paned), toolpicker);

    GtkWidget* layerpicker = create_layerpicker();
    gtk_paned_set_end_child(GTK_PANED(left_paned), layerpicker);

    // Part B
	GtkWidget* imageviewer = create_imageviewer();
    gtk_paned_set_end_child(GTK_PANED(main_paned), imageviewer);
