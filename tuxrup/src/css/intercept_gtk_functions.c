...
GtkApplication *appi = NULL;
// Function pointer types
typedef void (*gtk_css_provider_load_from_file_t)(GtkCssProvider*, GFile*);
static gtk_css_provider_load_from_file_t real_gtk_css_provider_load_from_file = NULL;



// Function to resolve symbols explicitly
static void resolve_symbols()
{
    void *handle = dlopen("libgtk-4.so", RTLD_LAZY);
    if (!handle) {
        fprintf(stderr, "[HOOK ERROR] Failed to open libgtk-4.so: %s\n", dlerror());
        return;
    }
    real_gtk_css_provider_load_from_file = dlsym(handle, "gtk_css_provider_load_from_file");


    if (!real_gtk_css_provider_load_from_file) {
        fprintf(stderr, "[HOOK ERROR] Could not find some necessary functions in libgtk-4.so!\n");
    }
...
void gtk_css_provider_load_from_file(GtkCssProvider *provider, GFile *file)
{
    ...
    fprintf(stderr, "[HOOK] gtk_css_provider_load_from_file() called\n");
    printf("\n[GTK HOOK] CSS File Loaded: %s\n\n", path);
    g_free(path);

    if (real_gtk_css_provider_load_from_file) {
        real_gtk_css_provider_load_from_file(provider, file);
    }
}
